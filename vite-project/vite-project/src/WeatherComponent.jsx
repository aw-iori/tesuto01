...
const WeatherComponent = () => {
  const [data, setData] = useState({ weatherData:[{timeSeries:[{areas:[{}]}]}],error: null }); 
  useEffect(() => {
  ...
  return (
    <div><h1>Weather Data</h1><pre>{JSON.stringify(
      data.weatherData[0].timeSeries[0].areas[0].area, null, 2)}</pre>
      <pre>{JSON.stringify(
      data.weatherData[0].timeSeries[0].areas[0].weathers, null, 2)}</pre>
      <pre>{JSON.stringify(
      data.weatherData[0].timeSeries[0].areas[0].waves, null, 2)}</pre>
    </div>
  );
 };
